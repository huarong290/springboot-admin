...
CREATE TABLE `sys_org` (
  ...
  `parent_ids` VARCHAR(255) NOT NULL DEFAULT '0' COMMENT '父级ID路径，例如 0/1/3',
...
CREATE TABLE `sys_menu` (
  `id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `menu_parent_id` BIGINT NOT NULL DEFAULT 0 COMMENT '父菜单ID，顶级为0',
  `menu_parent_ids` VARCHAR(255) NOT NULL DEFAULT '0' COMMENT '父级ID路径，例如 0/1/3',
  ...
  `menu_visible` TINYINT NOT NULL DEFAULT 1 COMMENT '是否显示：1显示 0隐藏',
  `menu_status` TINYINT NOT NULL DEFAULT 1 COMMENT '状态：1启用 0禁用',
  `delete_flag` TINYINT NOT NULL DEFAULT 0 COMMENT '是否删除：0未删除 1已删除',
  `version` INT NOT NULL DEFAULT 0 COMMENT '乐观锁版本号',
  `create_by` VARCHAR(64) NOT NULL DEFAULT 'system' COMMENT '创建人',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  ...
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_parent_name` (`menu_parent_id`, `menu_name`),
...
-- 初始化角色
-- =========================
INSERT INTO sys_role (id, role_name, role_code, role_description, role_status, delete_flag, version, create_by, create_time, update_by, update_time)
VALUES
(1, '超级管理员', 'super_admin', '系统超级管理员，拥有所有权限', 1, 0, 0, 'system', NOW(), 'system', NOW()),
(2, '管理员', 'admin', '系统管理员，拥有基础管理权限', 1, 0, 0, 'system', NOW(), 'system', NOW());

-- =========================
-- 初始化用户（密码需替换为加密后的值）
-- =========================
INSERT INTO sys_user (id, username, password, nickname, email, phone, avatar, status, dept_id, org_id, last_login_time, delete_flag, version, create_by, create_time, update_by, update_time)
VALUES
(1, 'system', '$2a$10$abcdefghijklmnopqrstuv', '系统用户', 'system@example.com', '13800000000', '', 1, NULL, NULL, NULL, 0, 0, 'system', NOW(), 'system', NOW()),
(2, 'admin', '$2a$10$abcdefghijklmnopqrstuv', '管理员用户', 'admin@example.com', '13900000000', '', 1, NULL, NULL, NULL, 0, 0, 'system', NOW(), 'system', NOW());
...
VALUES
(1, 1, 1, 0, 0, 'system', NOW(), 'system', NOW()), -- system → super_admin
(2, 2, 2, 0, 0, 'system', NOW(), 'system', NOW()); -- admin → admin

-- =========================
-- 初始化菜单
-- =========================
INSERT INTO sys_menu (id, menu_parent_id, menu_parent_ids, menu_name, menu_path, menu_component, menu_icon, menu_type, menu_sort, menu_visible, menu_status, delete_flag, version, create_by, create_time, update_by, update_time)
VALUES
(100, 0, '0', '系统管理', '/system', 'Layout', 'setting', 0, 1, 1, 1, 0, 0, 'system', NOW(), 'system', NOW()),
(101, 100, '0/100', '用户管理', '/system/user', 'system/user/index', 'user', 1, 1, 1, 1, 0, 0, 'system', NOW(), 'system', NOW()),
(102, 100, '0/100', '角色管理', '/system/role', 'system/role/index', 'team', 1, 2, 1, 1, 0, 0, 'system', NOW(), 'system', NOW()),
(103, 100, '0/100', '权限管理', '/system/permission', 'system/permission/index', 'lock', 1, 3, 1, 1, 0, 0, 'system', NOW(), 'system', NOW()),
(104, 100, '0/100', '菜单管理', '/system/menu', 'system/menu/index', 'menu', 1, 4, 1, 1, 0, 0, 'system', NOW(), 'system', NOW());

-- =========================
-- 角色菜单关联
-- =========================
INSERT INTO sys_role_menu (role_id, menu_id, delete_flag, version, create_by, create_time, update_by, update_time)
SELECT 1, id, 0, 0, 'system', NOW(), 'system', NOW() FROM sys_menu WHERE id IN (100,101,102,103,104);

INSERT INTO sys_role_menu (role_id, menu_id, delete_flag, version, create_by, create_time, update_by, update_time)
SELECT 2, id, 0, 0, 'system', NOW(), 'system', NOW() FROM sys_menu WHERE id IN (100,101,102,103,104);
...
VALUES
(201, 'user:manage', '用户管理权限', 1, 1, 0, 0, 'system', NOW(), 'system', NOW()),
...
SELECT 1, id, 0, 0, 'system', NOW(), 'system', NOW() FROM sys_permission;

INSERT INTO sys_role_permission (role_id, permission_id, delete_flag, version, create_by, create_time, update_by, update_time)
SELECT 2, id, 0, 0, 'system', NOW(), 'system', NOW() FROM sys_permission WHERE id IN (201,202,203,204);
...
VALUES
(101, 201, 0, 0, 'system', NOW(), 'system', NOW()),
