...
-- 初始化角色
INSERT INTO sys_role (id, role_name, role_code, role_description, role_status, delete_flag, create_by, create_time, update_by, update_time)
VALUES
(1, '超级管理员', 'super_admin', '系统超级管理员，拥有所有权限', 1, 0, 'system', NOW(), 'system', NOW()),
(2, '管理员', 'admin', '系统管理员，拥有基础管理权限', 1, 0, 'system', NOW(), 'system', NOW());

-- 初始化用户（密码需替换为加密后的值）
INSERT INTO sys_user (id, username, password, nickname, email, phone, avatar, status, dept_id, org_id, last_login_time, delete_flag, create_by, create_time, update_by, update_time)
VALUES
(1, 'system', '$2a$10$abcdefghijklmnopqrstuv', '系统用户', 'system@example.com', '13800000000', '', 1, NULL, NULL, NULL, 0, 'system', NOW(), 'system', NOW()),
(2, 'admin', '$2a$10$abcdefghijklmnopqrstuv', '管理员用户', 'admin@example.com', '13900000000', '', 1, NULL, NULL, NULL, 0, 'system', NOW(), 'system', NOW());

-- 用户角色关联
INSERT INTO sys_user_role (id, user_id, role_id, delete_flag, create_by, create_time, update_by, update_time)
VALUES
(1, 1, 1, 0, 'system', NOW(), 'system', NOW()), -- system → super_admin
(2, 2, 2, 0, 'system', NOW(), 'system', NOW()); -- admin → admin

-- 初始化菜单
INSERT INTO sys_menu (id, menu_parent_id, menu_name, menu_path, menu_component, menu_icon, menu_type, menu_permission, menu_sort, menu_visible, menu_status, delete_flag, create_by, create_time, update_by, update_time)
VALUES
(100, 0, '系统管理', '/system', 'Layout', 'setting', 0, '', 1, 1, 1, 0, 'system', NOW(), 'system', NOW()),
(101, 100, '用户管理', '/system/user', 'system/user/index', 'user', 1, 'user:manage', 1, 1, 1, 0, 'system', NOW(), 'system', NOW()),
(102, 100, '角色管理', '/system/role', 'system/role/index', 'team', 1, 'role:manage', 2, 1, 1, 0, 'system', NOW(), 'system', NOW()),
(103, 100, '权限管理', '/system/permission', 'system/permission/index', 'lock', 1, 'permission:manage', 3, 1, 1, 0, 'system', NOW(), 'system', NOW()),
(104, 100, '菜单管理', '/system/menu', 'system/menu/index', 'menu', 1, 'menu:manage', 4, 1, 1, 0, 'system', NOW(), 'system', NOW());

-- 角色菜单关联
INSERT INTO sys_role_menu (role_id, menu_id, delete_flag, create_by, create_time, update_by, update_time)
SELECT 1, id, 0, 'system', NOW(), 'system', NOW() FROM sys_menu WHERE id IN (100,101,102,103,104);
INSERT INTO sys_role_menu (role_id, menu_id, delete_flag, create_by, create_time, update_by, update_time)
SELECT 2, id, 0, 'system', NOW(), 'system', NOW() FROM sys_menu WHERE id IN (100,101,102,103,104);

-- 初始化权限点
INSERT INTO sys_permission (id, permission_code, permission_name, permission_type, permission_status, delete_flag, create_by, create_time, update_by, update_time)
VALUES
(201, 'user:manage', '用户管理权限', 1, 1, 0, 'system', NOW(), 'system', NOW()),
...
SELECT 1, id, 0, 'system', NOW(), 'system', NOW() FROM sys_permission;
INSERT INTO sys_role_permission (role_id, permission_id, delete_flag, create_by, create_time, update_by, update_time)
SELECT 2, id, 0, 'system', NOW(), 'system', NOW() FROM sys_permission WHERE id IN (201,202,203,204);

-- 菜单权限关联
INSERT INTO sys_menu_permission (menu_id, permission_id, delete_flag, create_by, create_time, update_by, update_time)
VALUES
(101, 201, 0, 'system', NOW(), 'system', NOW()),
